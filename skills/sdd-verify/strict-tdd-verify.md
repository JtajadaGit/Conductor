...
Apply the same assertion quality rules as the apply phase (see `strict-tdd.md` Assertion Quality section). Scan all test files created or modified during this change for:
- Banned patterns (tautologies, type-only, ghost loops, empty collections)
- Assertions that don't call production code
- Tests with zero meaningful assertions

Additionally check triangulation quality: flag WARNING if only 1 test case exists for a behavior with multiple spec scenarios, or if all test cases assert the same trivial value.

Flag each violation with file path, line reference, and the specific banned pattern matched. Classify: CRITICAL for tautologies and no-production-call assertions; WARNING for empty-collection and type-only violations.

Report in the Assertion Quality table (see Report Template Extension above). If zero issues: "**Assertion quality**: ✅ All assertions verify real behavior"
