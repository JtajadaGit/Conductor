...
3. Match relevant skills from the registry's **Compact Rules** by:
   ...
   - **Task context**: "review code" → framework/language skills; "create PR" → PR-related skills
...
- Launch **TWO** sub-agents (async, parallel — never sequential)
...
> **Platform note**: In Claude Code, use `delegate (async)`. In GitHub Copilot, use the `task` tool with `mode: "background"`.

### Pattern 2: Verdict Synthesis

The **orchestrator** (NOT a sub-agent) compares results after reading both sub-agent results:
...
▼
Launch Judge A + Judge B in parallel (async) — with Project Standards injected
▼
Wait for both to complete (read both sub-agent results)
▼
Synthesize verdict
│
├── No issues found?
...
- Judges MUST be launched as sub-agents (async) so they run in **parallel**
...
# Execution happens via launching sub-agents and reading their results.
```
