import{_ as e,c as t,d as n,f as r,g as i,h as a,i as o,l as s,m as c,n as l,o as u,p as d,r as f,t as p,u as m,v as h}from"./index-Gt37scUF.js";import"./status-pill-GGUJM3Ug.js";var g={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},_=e=>(...t)=>({_$litDirective$:e,values:t}),v=class{constructor(e){}get _$AU(){return this._$AM._$AU}_$AT(e,t,n){this._$Ct=e,this._$AM=t,this._$Ci=n}_$AS(e,t){return this.update(e,t)}update(e,t){return this.render(...t)}},{I:y}=h,b=e=>e.strings===void 0,x={},S=(e,t=x)=>e._$AH=t,C=_(class extends v{constructor(e){if(super(e),e.type!==g.PROPERTY&&e.type!==g.ATTRIBUTE&&e.type!==g.BOOLEAN_ATTRIBUTE)throw Error("The `live` directive is not allowed on child or event bindings");if(!b(e))throw Error("`live` bindings can only contain a single expression")}render(e){return e}update(e,[t]){if(t===i||t===a)return t;let n=e.element,r=e.name;if(e.type===g.PROPERTY){if(t===n[r])return i}else if(e.type===g.BOOLEAN_ATTRIBUTE){if(!!t===n.hasAttribute(r))return i}else if(e.type===g.ATTRIBUTE&&n.getAttribute(r)===t+``)return i;return S(e),t}}),w=class extends n{constructor(...e){super(...e),this.value=``,this.projId=``,this.placeholder=``,this.rows=3,this.open=!1,this.items=[],this.index=0,this.kind=`@`,this.query=``,this.tokStart=0,this.timer=0,this.seq=0,this.pendingCaret=null,this.skillsCache=null,this.skillsCacheProj=``}disconnectedCallback(){super.disconnectedCallback(),clearTimeout(this.timer)}emit(e){this.dispatchEvent(new CustomEvent(`cdr-input`,{detail:{value:e},bubbles:!0,composed:!0}))}close(){this.open=!1,this.items=[],clearTimeout(this.timer)}onInput(e){let t=e.target;this.emit(t.value);let n=t.value.slice(0,t.selectionStart).match(/(^|\s)(?:@([^\s@]*)|\/([^\s@/]*))$/);if(!n){this.close();return}let r=n[2]!==void 0;this.kind=r?`@`:`/`,this.query=r?n[2]:n[3],this.tokStart=t.selectionStart-this.query.length-1,this.open=!0,this.index=0,clearTimeout(this.timer),this.timer=window.setTimeout(()=>void this.fetchItems(),130)}async fetchItems(){let e=++this.seq,t=this.projId;try{let n=new URLSearchParams;if(t&&n.set(`project`,t),this.kind===`@`){n.set(`q`,this.query);let t=await(await fetch(`/api/files?`+n)).json();if(e!==this.seq)return;this.items=(t.files||[]).slice(0,30).map(e=>({v:e,label:e}))}else{if((!this.skillsCache||this.skillsCacheProj!==t)&&(this.skillsCache=(await(await fetch(`/api/skills?`+n)).json()).skills||[],this.skillsCacheProj=t),e!==this.seq)return;let r=this.query.toLowerCase();this.items=(this.skillsCache||[]).filter(e=>!r||e.name.toLowerCase().includes(r)).slice(0,20).map(e=>({v:e.name,label:e.name,sub:e.title}))}this.index=0}catch{this.items=[]}}onKey(e){this.open&&(e.key===`ArrowDown`?(e.preventDefault(),this.items.length&&(this.index=(this.index+1)%this.items.length)):e.key===`ArrowUp`?(e.preventDefault(),this.items.length&&(this.index=(this.index-1+this.items.length)%this.items.length)):(e.key===`Enter`||e.key===`Tab`)&&this.items.length?(e.preventDefault(),this.insert(this.items[this.index])):e.key===`Escape`&&(e.preventDefault(),this.close()))}insert(e){let t=this.querySelector(`textarea`);if(!t)return;let n=t.selectionStart,r=this.kind===`@`&&/\s/.test(e.v)?`"`+e.v+`"`:e.v,i=this.kind+r,a=t.value.slice(0,this.tokStart)+i+` `+t.value.slice(n),o=this.tokStart+i.length+1;this.value=a,this.pendingCaret=o,this.emit(a),this.close()}updated(){if(this.pendingCaret==null)return;let e=this.querySelector(`textarea`);e&&(e.focus(),e.setSelectionRange(this.pendingCaret,this.pendingCaret)),this.pendingCaret=null}render(){return e`
      ...
      <div class="launch-init" style="display:flex;flex-wrap:wrap;align-items:center;gap:.8rem;padding:1rem 1.1rem;border:1px solid var(--bd);border-left:3px solid var(--accent);border-radius:var(--r);background:var(--accentbg);color:var(--tx)">
      ...
      </div>`}async fetchXray(){if(this.xrayBusy)return;let e=this.projId;this.xrayBusy=!0,this.xrayError=``;try{let t=await fetch(`/api/explain`+(e?`?projectId=`+encodeURIComponent(e):``)),n=await t.json().catch(()=>null);n?.ok?(this.xray=n.capabilities??[],this.xrayFor=e):(this.xray=null,this.xrayError=n?.error??`el análisis no respondió (HTTP ${t.status})`)}catch{this.xray=null,this.xrayError=`No se pudo conectar con el servidor local. Comprueba que la aplicación está en ejecución.`}finally{this.xrayBusy=!1}}xrayFresh(){return this.xray!==null&&this.xrayFor===this.projId}ensureXray(){!this.xrayFresh()&&!this.xrayBusy&&this.fetchXray()}xrayBody(){if(this.xrayBusy)return s(`Analizando tu código`);if(this.xrayError)return e`
        <p class="xray-err" role="alert">${this.xrayError}</p>
        <button type="button" class="btn sm sec" @click=${()=>void this.fetchXray()} aria-label="Reintentar la radiografía del repositorio">Reintentar</button>`;if(!this.xrayFresh())return e`
        ...
        <button type="button" class="btn" @click=${()=>void this.fetchXray()} aria-label="Analizar el repositorio enfocado — gratis, sin gastar tokens">Analizar mi repo — gratis</button>`;let t=this.xray??[];if(!t.length)return e`<p class="xray-note" role="status">Aún no reconozco capacidades en este repo — lanza tu primera feature y el índice crecerá.</p>`;let n=t.slice(0,12),r=t.length-n.length;return e`
      <p class="xray-head" role="status">conductor ya entiende <strong>${t.length} ${t.length===1?`capacidad`:`capacidades`}</strong> de tu código.</p>
      <ul class="xray-list">
        ${n.map(t=>e`<li class="xray-row"><code class="xray-id">${t.id}</code><span class="xray-n">${t.endpoints} endpoint${t.endpoints===1?``:`s`}</span><span class="xray-n">${t.files} fichero${t.files===1?``:`s`}</span></li>`)}
      </ul>
      ${r>0?e`<p class="muted xray-more">…y ${r} más</p>`:a}
      <p class="xray-note">Radiografía determinista: 0 LLM · 0 tokens · 0 red. Sobre este mapa construye el pipeline — la spec de tu primera feature parte de aquí, no de cero.</p>`}xrayCard(){return e`
      <section class="xray-card" aria-labelledby="xray-title">
        ...
        ${this.xrayBody()}
      </section>`}xrayFold(){return e`
      <details class="launch-fold xray-fold" @toggle=${e=>{e.target.open&&this.ensureXray()}}>
      ...
      <div class="launch-plan" style="margin:.1rem 0 .2rem;padding:.7rem .9rem;border-left:3px solid var(--accent);background:var(--accentbg);border-radius:7px;color:var(--tx)">
        ...
        <ul role="group" aria-label="Fases SDD del run" style="margin:.35rem 0 0;padding:0;list-style:none;font-size:.85rem;line-height:1.5">
          ${E.map(t=>{let n=T[t],r=D.includes(t),o=r||i.has(t);return e`<li style="padding:.12rem 0">
      ...
      </div>`}render(){let t=this.scopeProjects(),n=this.metrics(t),r=this.modelOptions(),i=t.flatMap(e=>(e.changes??[]).filter(e=>u(e.verdict)===`CURSO`).map(t=>({p:e,c:t}))),o=t.flatMap(e=>(e.changes??[]).filter(e=>u(e.verdict)!==`CURSO`).map(t=>({p:e,c:t}))),s=e`
      <form class="launch-form" @submit=${e=>void this.launch(e)}>
        ...
        <div class="launch-meta">
          ...
          ${this.est?e`<details class="lm-estd"><summary class="lm-est">≈ ${l(this.est.total)} tokens · ${this.est.rows.length} fases <span class="muted">· preflight, sin API</span></summary>
            <table class="est-tab">${this.est.rows.map(t=>e`<tr><td>${t.phase}</td><td>↓ ${l(t.estIn)}</td><td>↑ ${l(t.estOut)}</td></tr>`)}</table>
            ${this.est.saved>0?e`<p class="est-saved">Ahorro estimado de ${l(this.est.saved)} tokens de entrada <span class="muted">(el planner no relee el repositorio)</span></p>`:a}
          </details>`:a}
        </div>
        ${r.length?e`
        <details class="inst-panel" style="margin-top:.6rem">
    ...
    `,c=this.projects.find(e=>e.id===this.projId)??null,d=c&&c.openspec===!1?this.initPanel(c):s,f=this.attention();return e`
      ...
      <div class="apphdr"><h1>${c?c.name:`conductor`}</h1></div>
      ...
      <details class="launch-fold metrics">
        ...
        <div class="cards" style="margin-top:.9rem">
          ...
          <div class="card"><small>Tokens entrada ↓</small><span>${l(n.tin)}</span></div>
          <div class="card"><small>Tokens salida ↑</small><span>${l(n.tout)}</span></div>
          ...
          ${this.usage?e`<div class="card"><small>Uso total qwen</small><span>$${this.usage.spend.toFixed(2)}${this.usage.budget?e` <span class="muted" style="font-size:.8rem;font-weight:500">/ $${this.usage.budget.toFixed(0)}</span>`:a}</span>${this.usage.budget?e`<div class="pbar ${this.usage.spend/this.usage.budget>.8?`warn`:``}"><i style="width:${Math.min(100,this.usage.spend/this.usage.budget*100)}%"></i></div>`:a}</div>`:a}
        </div>
      </details>
    ...
    `}hitRow(t){let n=e`<status-pill .verdict=${t.verdict}></status-pill><span class="nm">${t.name}</span>${t.archived?e`<span class="tag">📦</span>`:a}${t.project?e`<span class="proj">📁 ${t.project}</span>`:a}<span class="muted snip">…${t.snippet}…</span>`;return e`<div class="hit-row">${t.archived?n:e`<a class="hit-main" href="/run/${t.projectId??this.defProjId}/${t.name}">${n}</a>`}</div>`}launchModelSummary(){let t=e=>e.replace(/^(byok|copilot):/,``),n=[this.mPlanner,this.mCoder,this.mReviewer];return n.some(Boolean)?e`<span class="lm-model" title="modelo elegido por fase — cámbialo en «Modelo por fase»">Modelo: <b>${n.every(e=>e===n[0])?t(n[0]):`planner ${t(this.mPlanner)||`—`} · coder ${t(this.mCoder)||`—`} · reviewer ${t(this.mReviewer)||`—`}`}</b></span>`:a}mixNote(){let t=[...new Set([this.mPlanner,this.mCoder,this.mReviewer].map(e=>e?e.startsWith(`byok:`)?`qwen`:e.startsWith(`copilot:`)?`Copilot`:`sesión`:``).filter(Boolean))];return t.length<2?a:e`<div class="muted" style="font-size:.72rem;margin-top:.5rem">proveedores: ${t.join(` + `)}</div>`}roleSelect(t,n,r){let i=this.models,o=i?.copilot??[],s=i?.byok??[],c=!!i?.byokCreds,l=e=>e.startsWith(`claude`)?`Claude`:e.startsWith(`gpt`)?`GPT`:e.startsWith(`gemini`)?`Gemini`:`Otros`,u=[`Claude`,`GPT`,`Gemini`,`Otros`].map(e=>[e,o.filter(t=>l(t)===e).sort((e,t)=>t.localeCompare(e))]).filter(([,e])=>e.length),d=i?.copilotPending;return e`<label class="fl" style="flex:1">${t}<select .value=${n} title=${i?`Copilot: ${i.copilotSource} · qwen: ${i.byokSource}`:``} @change=${e=>r(e.target.value)}>
      ...
      ${c&&s.length?e`<optgroup label="qwen · LiteLLM">${s.map(t=>e`<option value="byok:${t}">${t}</option>`)}</optgroup>`:a}
    </select></label>`}runRow(t,n){return e`
      <div class="run-row ${u(n.verdict)===`CURSO`?`run-active`:``}">
        <div class="run-l">
          ...
          <div class="run-foot">
            <span class="meta">${n.phases} fases${(n.tokens?.in??0)+(n.tokens?.out??0)>0?e` · ↓ ${l(n.tokens?.in)} entrada · ↑ ${l(n.tokens?.out)} salida`:a}</span>
            ${n.resumable?e`<button class="btn sm resume" @click=${()=>void this.resume(t,n)} aria-label="reanudar ${n.name}">⏯ Reanudar</button>`:a}
            ${!n.resumable&&u(n.verdict)!==`CURSO`&&n.request?e`<button class="btn sm sec" @click=${()=>this.reuse(n)} title="rellena el formulario con esta petición para lanzar una variante">↺ Reutilizar</button>`:a}
      ...
      </div>`}};p([r()],O.prototype,`projects`,void 0),p([r()],O.prototype,`gh`,void 0),p([r()],O.prototype,`usage`,void 0),p([r()],O.prototype,`version`,void 0),p([r()],O.prototype,`models`,void 0),p([r()],O.prototype,`req`,void 0),p([r()],O.prototype,`name`,void 0),p([r()],O.prototype,`complexity`,void 0),p([r()],O.prototype,`auto`,void 0),p([r()],O.prototype,`projId`,void 0),p([r()],O.prototype,`mPlanner`,void 0),p([r()],O.prototype,`mCoder`,void 0),p([r()],O.prototype,`mReviewer`,void 0),p([r()],O.prototype,`preset`,void 0),p([r()],O.prototype,`busy`,void 0),p([r()],O.prototype,`error`,void 0),p([r()],O.prototype,`byokUrl`,void 0),p([r()],O.prototype,`byokKey`,void 0),p([r()],O.prototype,`byokSaving`,void 0),p([r()],O.prototype,`byokMsg`,void 0),p([r()],O.prototype,`est`,void 0),p([r()],O.prototype,`phaseSel`,void 0),p([r()],O.prototype,`runTests`,void 0),p([r()],O.prototype,`initBusy`,void 0),p([r()],O.prototype,`q`,void 0),p([r()],O.prototype,`hits`,void 0),p([r()],O.prototype,`archived`,void 0),p([r()],O.prototype,`xray`,void 0),p([r()],O.prototype,`xrayBusy`,void 0),p([r()],O.prototype,`xrayError`,void 0),O=p([c(`panel-screen`)],O);export{O as PanelScreen};