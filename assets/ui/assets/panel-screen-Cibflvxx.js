import{_ as e,a as t,d as n,f as r,g as i,h as a,l as o,m as s,n as c,p as l,r as u,s as d,t as f,u as p}from"./index-Bfc-rE4S.js";import"./status-pill-DQe8w0Ss.js";var m={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},h=e=>(...t)=>({_$litDirective$:e,values:t}),g=class{constructor(e){}get _$AU(){return this._$AM._$AU}_$AT(e,t,n){this._$Ct=e,this._$AM=t,this._$Ci=n}_$AS(e,t){return this.update(e,t)}update(e,t){return this.render(...t)}},{I:_}=e,v=e=>e.strings===void 0,y={},b=(e,t=y)=>e._$AH=t,x=h(class extends g{constructor(e){if(super(e),e.type!==m.PROPERTY&&e.type!==m.ATTRIBUTE&&e.type!==m.BOOLEAN_ATTRIBUTE)throw Error("The `live` directive is not allowed on child or event bindings");if(!v(e))throw Error("`live` bindings can only contain a single expression")}render(e){return e}update(e,[t]){if(t===a||t===s)return t;let n=e.element,r=e.name;if(e.type===m.PROPERTY){if(t===n[r])return a}else if(e.type===m.BOOLEAN_ATTRIBUTE){if(!!t===n.hasAttribute(r))return a}else if(e.type===m.ATTRIBUTE&&n.getAttribute(r)===t+``)return a;return b(e),t}}),S=class extends p{constructor(...e){super(...e),this.value=``,this.projId=``,this.placeholder=``,this.rows=3,this.open=!1,this.items=[],this.index=0,this.kind=`@`,this.query=``,this.tokStart=0,this.timer=0,this.seq=0,this.pendingCaret=null,this.skillsCache=null}disconnectedCallback(){super.disconnectedCallback(),clearTimeout(this.timer)}emit(e){this.dispatchEvent(new CustomEvent(`cdr-input`,{detail:{value:e},bubbles:!0,composed:!0}))}close(){this.open=!1,this.items=[],clearTimeout(this.timer)}onInput(e){let t=e.target;this.emit(t.value);let n=t.value.slice(0,t.selectionStart).match(/(^|\s)(?:@([^\s@]*)|\/([^\s@/]*))$/);if(!n){this.close();return}let r=n[2]!==void 0;this.kind=r?`@`:`/`,this.query=r?n[2]:n[3],this.tokStart=t.selectionStart-this.query.length-1,this.open=!0,this.index=0,clearTimeout(this.timer),this.timer=window.setTimeout(()=>void this.fetchItems(),130)}async fetchItems(){let e=++this.seq;try{let t=new URLSearchParams;if(this.projId&&t.set(`project`,this.projId),this.kind===`@`){t.set(`q`,this.query);let n=await(await fetch(`/api/files?`+t)).json();if(e!==this.seq)return;this.items=(n.files||[]).slice(0,30).map(e=>({v:e,label:e}))}else{if(this.skillsCache||=(await(await fetch(`/api/skills?`+t)).json()).skills||[],e!==this.seq)return;let n=this.query.toLowerCase();this.items=(this.skillsCache||[]).filter(e=>!n||e.name.toLowerCase().includes(n)).slice(0,20).map(e=>({v:e.name,label:e.name,sub:e.title}))}this.index=0}catch{this.items=[]}}onKey(e){this.open&&(e.key===`ArrowDown`?(e.preventDefault(),this.items.length&&(this.index=(this.index+1)%this.items.length)):e.key===`ArrowUp`?(e.preventDefault(),this.items.length&&(this.index=(this.index-1+this.items.length)%this.items.length)):(e.key===`Enter`||e.key===`Tab`)&&this.items.length?(e.preventDefault(),this.insert(this.items[this.index])):e.key===`Escape`&&(e.preventDefault(),this.close()))}insert(e){let t=this.querySelector(`textarea`);if(!t)return;let n=t.selectionStart,r=this.kind===`@`&&/\s/.test(e.v)?`"`+e.v+`"`:e.v,i=this.kind+r,a=t.value.slice(0,this.tokStart)+i+` `+t.value.slice(n),o=this.tokStart+i.length+1;this.value=a,this.pendingCaret=o,this.emit(a),this.close()}updated(){if(this.pendingCaret==null)return;let e=this.querySelector(`textarea`);e&&(e.focus(),e.setSelectionRange(this.pendingCaret,this.pendingCaret)),this.pendingCaret=null}render(){return i`
      <div class="mi-wrap">
        <textarea class="mi-ta" rows=${this.rows} placeholder=${this.placeholder} .value=${x(this.value)} required
          ...
          aria-controls="mi-menu" aria-activedescendant=${this.open&&this.items.length?`mi-opt-`+this.index:s}
        ...
        ${this.open?i`
          <ul id="mi-menu" class="mi-menu" role="listbox" aria-label=${this.kind===`@`?`ficheros`:`skills`}>
            ...
            ${this.items.length?this.items.map((e,t)=>i`
      ...
      <div class="launch-init" style="padding:1rem 1.1rem;border:1px solid var(--bd);border-left:3px solid var(--accent);border-radius:var(--r);background:var(--accentbg);color:var(--tx)">
        ...
        ${this.initMsg?i`<p role="alert" style="margin:.55rem 0 0;font-size:.82rem;color:var(--bad)">${this.initMsg}</p>`:s}
      </div>`}async resume(e,t){let n=await this.api.resumeNamed(t.name,e.id);n.ok&&n.url&&o.go(n.url)}metrics(e){let n=e.flatMap(e=>e.changes??[]);return{total:n.length,green:n.filter(e=>t(e.verdict)===`GREEN`).length,curso:n.filter(e=>t(e.verdict)===`CURSO`).length,tin:n.reduce((e,t)=>e+(t.tokens?.in??0),0),tout:n.reduce((e,t)=>e+(t.tokens?.out??0),0)}}activeProject(){return this.projects.find(e=>e.id===this.projId)??null}scopeProjects(){if(this.showAll)return this.projects;let e=this.activeProject();return e?[e]:this.projects}switchProject(e){!e||e===this.projId||(this.projId=e,this.persistActive(),this.refreshChanges())}projectSwitcher(){return this.projects.length<=1?s:i`<label class="fl" style="display:inline-flex;flex-flow:row wrap;align-items:center;gap:.45rem;margin:0 0 1rem;font-size:.82rem">Proyecto
      <select aria-label="Proyecto activo" @change=${e=>this.switchProject(e.target.value)} style="min-width:12rem">
        ${this.projects.map(e=>i`<option value=${e.id} ?selected=${e.id===this.projId}>${e.name}${e.openspec===!1?` · sin inicializar`:``}</option>`)}
      ...
      <div class="launch-plan" style="margin:.1rem 0 .2rem;padding:.7rem .9rem;border-left:3px solid var(--accent);background:var(--accentbg);border-radius:7px;color:var(--tx)">
        ...
        <ul role="group" aria-label="Fases SDD del run" style="margin:.35rem 0 0;padding:0;list-style:none;font-size:.85rem;line-height:1.5">
          ${w.map(e=>{let t=C[e],n=T.includes(e),a=n||r.has(e);return i`<li style="padding:.12rem 0">
      ...
      </div>`}render(){let e=this.scopeProjects(),n=this.metrics(e),r=this.modelOptions(),a=e.flatMap(e=>(e.changes??[]).filter(e=>t(e.verdict)===`CURSO`).map(t=>({p:e,c:t}))),o=e.flatMap(e=>(e.changes??[]).filter(e=>t(e.verdict)!==`CURSO`).map(t=>({p:e,c:t}))),l=i`
      <form class="launch-form" @submit=${e=>void this.launch(e)}>
        ...
        <div class="launch-meta">
          ...
          ${this.est?i`<details class="lm-estd"><summary class="lm-est">≈ ${c(this.est.total)} tokens · ${this.est.rows.length} fases <span class="muted">· preflight, sin API</span></summary>
            <table class="est-tab">${this.est.rows.map(e=>i`<tr><td>${e.phase}</td><td>↓ ${c(e.estIn)}</td><td>↑ ${c(e.estOut)}</td></tr>`)}</table>
            ${this.est.saved>0?i`<p class="est-saved">Ahorro estimado de ${c(this.est.saved)} tokens de entrada <span class="muted">(el planner no relee el repositorio)</span></p>`:s}
          </details>`:s}
        </div>
        ${r.length?i`
        <details class="inst-panel" style="margin-top:.6rem">
      ...
      ${this.error?i`<p role="alert" style="color:var(--bad)">${this.error}</p>`:s}
    `,u=this.projects.find(e=>e.id===this.projId)??null,d=u&&u.openspec===!1?this.initPanel(u):l;return i`
      <div class="apphdr"><h1>Panel</h1></div>
      <p class="subhead">
        ${u?i`Proyecto activo: <strong style="font-weight:600;color:var(--tx)">${u.name}</strong> <span style="opacity:.7">· ${u.root}</span>`:i`${this.projects.length} proyecto${this.projects.length===1?``:`s`}`}
        ${this.projects.length>1?i` · <button type="button" @click=${()=>{this.showAll=!this.showAll}} style="background:none;border:none;padding:0;font:inherit;color:var(--accent);cursor:pointer;text-decoration:underline">${this.showAll?`ver solo este`:`ver todos (${this.projects.length})`}</button>`:s}
        · ${n.total} run${n.total===1?``:`s`}${this.showAll?` · todos`:``}${this.version?i` · <span title="versión del motor en uso">motor v${this.version}</span>`:s}
      </p>
      ...
      <div class="cards">
        ...
        ${n.curso>0?i`<div class="card warn"><small>En curso</small><span>${n.curso}</span></div>`:s}
      ...
      ${n.total===0?i`<p class="muted" style="margin-top:.5rem">Aún no hay runs. Lanza el primero arriba.</p>`:s}
    ...
    </select></label>`}runRow(e,n){return i`
      <div class="run-row ${t(n.verdict)===`CURSO`?`run-active`:``}">
        <div class="run-l">
          <a class="main" href="/run/${e.id}/${n.name}">
            <span class="nm">${n.name} <status-pill .verdict=${n.pending?`EN PAUSA`:n.verdict}></status-pill>${n.pending?i`<span class="pill CURSO" title="el run espera tu revisión">⏸ tu decisión</span>`:s}</span>
            <span class="rq">${n.request}</span>
            <span class="proj">📁 ${e.name}</span>
          </a>
          <div class="run-foot">
            <span class="meta">${n.phases} fases · ↓ ${c(n.tokens?.in)} entrada · ↑ ${c(n.tokens?.out)} salida</span>
            ${n.resumable?i`<button class="btn sm resume" @click=${()=>void this.resume(e,n)} aria-label="reanudar ${n.name}">⏯ Reanudar</button>`:s}
            ${n.hasDashboard?i`<a class="btn sm dash" href="/artifact/${e.id}/${n.name}/dashboard.html" target="_blank" aria-label="informe de ${n.name}">📊 Informe</a>`:s}
            ${n.phases>0?i`<a class="btn sm aiact" href="/api/run/${e.id}/${n.name}/aiact" target="_blank" aria-label="AI Act de ${n.name}">🛡 AI Act</a>`:s}
          </div>
        </div>
        <a class="run-open" href="/run/${e.id}/${n.name}" tabindex="-1" aria-hidden="true"><span class="chev" aria-hidden="true"></span></a>
      </div>`}};f([n()],E.prototype,`projects`,void 0),f([n()],E.prototype,`gh`,void 0),f([n()],E.prototype,`usage`,void 0),f([n()],E.prototype,`version`,void 0),f([n()],E.prototype,`models`,void 0),f([n()],E.prototype,`req`,void 0),f([n()],E.prototype,`name`,void 0),f([n()],E.prototype,`complexity`,void 0),f([n()],E.prototype,`auto`,void 0),f([n()],E.prototype,`projId`,void 0),f([n()],E.prototype,`mPlanner`,void 0),f([n()],E.prototype,`mCoder`,void 0),f([n()],E.prototype,`mReviewer`,void 0),f([n()],E.prototype,`preset`,void 0),f([n()],E.prototype,`busy`,void 0),f([n()],E.prototype,`showAll`,void 0),f([n()],E.prototype,`error`,void 0),f([n()],E.prototype,`byokUrl`,void 0),f([n()],E.prototype,`byokKey`,void 0),f([n()],E.prototype,`byokSaving`,void 0),f([n()],E.prototype,`byokMsg`,void 0),f([n()],E.prototype,`est`,void 0),f([n()],E.prototype,`phaseSel`,void 0),f([n()],E.prototype,`runTests`,void 0),f([n()],E.prototype,`initBusy`,void 0),f([n()],E.prototype,`q`,void 0),f([n()],E.prototype,`hits`,void 0),f([n()],E.prototype,`archived`,void 0),E=f([l(`panel-screen`)],E);export{E as PanelScreen};