import{_ as e,a as t,c as n,d as r,f as i,h as a,l as o,m as s,n as c,o as l,p as u,t as d}from"./index-Du_WZ-IR.js";import"./status-pill-DIFsvKie.js";var f=class{constructor(e,t,n){this.base=e,this.onState=t,this.onError=n,this.timer=null,this.abort=null,this.lastHash=``,this.stopped=!0}start(){this.stopped=!1,this.lastHash=``,this.tick()}stop(){this.stopped=!0,this.timer!=null&&(clearTimeout(this.timer),this.timer=null),this.abort&&=(this.abort.abort(),null)}schedule(e){this.stopped||(this.timer=setTimeout(()=>void this.tick(),e))}async tick(){if(!this.stopped){this.abort=new AbortController;try{let e=await(await fetch(this.base+`state`,{signal:this.abort.signal})).json();if(this.stopped)return;if(!e||typeof e!=`object`||Array.isArray(e)){this.schedule(3e3);return}let t={...e,phases:Array.isArray(e.phases)?e.phases:[]},n=this.hashOf(t);if(n!==this.lastHash&&(this.lastHash=n,this.onState(t)),t.done){this.stop();return}this.schedule(t.pending?5e3:2e3)}catch(e){if(this.stopped)return;e.name!==`AbortError`&&this.onError?.(e),this.schedule(3e3)}}}hashOf(e){let{now:t,...n}=e;return JSON.stringify(n)}},p={explore:`🔍`,propose:`📝`,clarify:`❓`,spec:`📐`,design:`🧩`,tasks:`🗂️`,apply:`🛠️`,test:`🧪`,fix:`🔧`,verify:`🛡️`};function m(e){return p[e]??`•`}function h(e,t){let n=(e??``).toLowerCase();return t===`byok`||/qwen|deepseek|glm/.test(n)?`🔑`:/opus/.test(n)?`🧠`:/sonnet/.test(n)?`🎼`:/haiku/.test(n)?`⚡`:/gpt/.test(n)?`🤖`:`💼`}var g=class extends r{constructor(...e){super(...e),this.apiBase=`/api/`,this.phase=``,this.text=``,this.loaded=!1}async load(){if(!this.loaded){this.loaded=!0,this.text=`…`;try{let e=await(await fetch(this.apiBase+`raw?phase=`+encodeURIComponent(this.phase))).text();this.text=e&&e!==`no encontrado`?e.trim():`No se registró salida del modelo para esta fase.`}catch{this.text=`No se pudo cargar la salida del modelo.`}}}render(){return e`<details class="raw" @toggle=${e=>{e.target.open&&this.load()}}>
      ...
      </div>`}};d([u({attribute:!1})],_.prototype,`cost`,void 0),_=d([s(`model-breakdown`)],_);var v=class extends r{constructor(...e){super(...e),this.apiBase=`/api/`,this.change=``,this.projId=``,this.phaseId=``,this.s=null,this.err=``,this.selected=new Set,this.note=``,this.hotModel=``,this.models=null,this.files=null,this.busy=``,this.actionErr=``,this.archivedMsg=``,this.redoTarget=``,this.redoErr=``,this.copiedReceipt=!1,this.filesSig=``,this.pendingKey=``,this.api=new n(`/api/`),this.poller=null,this.activeBase=``,this.stopping=!1}updated(e){this.apiBase&&this.apiBase!==this.activeBase&&(this.activeBase=this.apiBase,this.restart())}disconnectedCallback(){super.disconnectedCallback(),this.poller?.stop(),clearTimeout(this.receiptTimer),document.title=`conductor`}restart(){this.poller?.stop(),this.s=null,this.err=``,this.files=null,this.filesSig=``,this.stopping=!1,this.pendingKey=``,this.selected=new Set,this.note=``,this.hotModel=``,this.redoTarget=``,this.redoErr=``,this.api=new n(this.apiBase),this.poller=new f(this.apiBase,e=>{e.done&&(this.stopping=!1);let t=e.pending?`${e.pending.before}|${(e.pending.findings||[]).length}`:``;t!==this.pendingKey&&(this.pendingKey=t,t&&(this.selected=new Set,this.note=``,this.hotModel=``,this.redoTarget=``,this.redoErr=``)),this.s=e,this.maybeFetchFiles(e),document.title=e.pending?`⏸ tu decisión — conductor`:`conductor`},e=>{this.err=e.message}),this.poller.start(),new n(`/api/`).models().then(e=>{this.models=e}).catch(()=>{})}async maybeFetchFiles(e){let t=(e.verdict??``)+`|`+e.phases.filter(e=>e.ok).length;if(t!==this.filesSig)try{this.files=await this.api.runFiles(),this.filesSig=t}catch{}}hotModelSelect(){let t=this.models,n=t?.copilot??[],r=t?.byok??[],i=!!t?.byokCreds;return e`<label class="fl">Modelo de esta fase<select .value=${this.hotModel} title=${t?`Copilot: ${t.copilotSource} · LiteLLM: ${t.byokSource}`:``} @change=${e=>{this.hotModel=e.target.value}}>
      ...
      <p class="subhead">${t.project||`—`}${t.branch?e` <span class="chip-branch" title="rama de git del proyecto">⎇ ${t.branch}</span>`:``}</p>
      <div class="actbar">
        <a class="btn sm sec" href=${this.sessionHref()}>📃 Ver sesión</a>
        ${(t.done||t.verdict===`INTERRUMPIDO`)&&l(t.verdict)!==`GREEN`?e`<button class="btn sm resume" ?disabled=${this.busy===`resume`} @click=${()=>void this.resumeRun()} title=${t.verdict===`INTERRUMPIDO`?`El proceso del run murió sin cerrar (¿equipo suspendido / terminal cerrada?) — reanuda desde la última fase completada`:`reanudar el run`}>${this.busy===`resume`?`Reanudando…`:`↻ Reanudar`}</button>`:a}
        ...
        ${t.done&&l(t.verdict)===`GREEN`?e`<button class="btn sm receipt" ?disabled=${this.busy===`receipt`} aria-live="polite" @click=${()=>void this.copyReceipt()}>${this.copiedReceipt?`Copiado ✓`:this.busy===`receipt`?`Copiando…`:`📋 Copiar descripción de PR`}</button>`:a}
      ...
      ${t.verdict===`INTERRUMPIDO`?e`<p class="alert warn" role="status"><span class="al-ic" aria-hidden="true">⏸</span><span>El proceso del run murió sin cerrar (¿equipo suspendido?). Nada se ha perdido: <b>↻ Reanudar</b> continúa desde la última fase completada.</span></p>`:a}
    ...
    `:o(`Cargando run`)}requestBox(t){return e`<details class="req" ?open=${!(t.request&&t.request.length>90)}>
    ...
    </div>`:a}specDomain(e){for(let t of e.phases){if(t.phase!==`spec`||!Array.isArray(t.files))continue;let e=t.files.find(e=>/specs[\\/].+[\\/]spec\.md$/i.test(e.p));if(e){let t=e.p.replace(/\\/g,`/`).match(/specs\/([^/]+)\/spec\.md$/i);if(t)return t[1]}}return null}viewSpecDiff(e){window.dispatchEvent(new CustomEvent(`cdr-view`,{detail:{apiBase:this.apiBase,kind:`specdiff`,path:e}}))}reviewArtifacts(e){let t={explore:`exploration.md`,propose:`proposal.md`,clarify:`questions.md`,design:`design.md`,tasks:`tasks.md`,apply:`apply-report.md`},n=[];for(let r of e.phases)if(r.ok)if(r.phase===`spec`){let e=Array.isArray(r.files)?r.files.find(e=>/specs[\\/].+[\\/]spec\.md$/i.test(e.p)):void 0,t=e?e.p.replace(/\\/g,`/`).replace(/^.*openspec\/changes\/[^/]+\//,``):``;t&&t.startsWith(`specs/`)&&n.push({label:`spec.md`,path:t})}else t[r.phase]&&n.push({label:t[r.phase],path:t[r.phase]});return n.filter((e,t)=>n.findIndex(t=>t.path===e.path)===t)}pendingCard(t,n){let r=this.reviewArtifacts(n),i=(t.findings??[]).map(e=>typeof e==`string`?{message:e}:e),o=this.selected.size,s=e=>e===`error`||e===`breaking`,c=t.before===`fix`?[]:this.redoOptions(n);return e`<section class="decision">
      ...
      <div class="decision-body">
        ${r.length?e`<div class="decision-arts"><span class="ctx-lbl">Revisar</span>${r.map(t=>e`<button type="button" class="lnk" title="abrir ${t.path} (editable)" @click=${()=>this.viewArtifact(t.path)}>📄 ${t.label}</button>`)}${this.specDomain(this.s)?e`<button type="button" class="lnk" title="diff del delta contra la spec viva promovida" @click=${()=>this.viewSpecDiff(this.specDomain(this.s))}>± vs spec viva</button>`:a}</div>`:a}
        ...
        <div class="decision-form">
          <label class="fl df-note"><span>Instrucción para esta fase <span class="opt">· opcional</span></span>
            <textarea class="pend-note" rows="2" .value=${this.note} @input=${e=>{this.note=e.target.value}} placeholder="p. ej. «usa el servicio de auth existente, no crees otro»" aria-describedby=${c.length?`df-help`:a}></textarea>
            ${c.length?e`<span class="df-help" id="df-help">Con <b>Aprobar</b> viaja a <b>${t.before}</b> · con <b>Rehacer</b> viaja a la fase de planificación que elijas (las posteriores se regeneran y el run vuelve a pausar aquí).</span>`:a}
          </label>
          ${this.hotModelSelect()}
        </div>
        <div class="decision-actions">
          ...
          ${c.length?e`<div class="redo-inline">
      ...
      ${d&&d.byok_phases>0?e`<span class="st-ok" title="fases vía LiteLLM — 0 AI Credits (↓${c(d.byok_in)} ↑${c(d.byok_out)} tokens fuera de Copilot)">LiteLLM ${d.byok_phases}/${d.byok_phases+d.copilot_phases} · 0 AIC</span>`:a}
    ...
    </details>`}estFor(e){let t=this.s?.estimate?.phases;if(!Array.isArray(t))return null;let n=t.find(t=>t.phase===e);return n?{estIn:n.estIn,estOut:n.estOut}:null}phaseCard(n){return e`<div class="ph ${n.ok?`done`:`bad`}">
      <div class="row">
        ...
        ${n.fallback?e`<span class="badge warn" title="pedido ${n.fallback.from} → reserva ${n.fallback.to} tras ${n.fallback.afterKind} (opt-in fallback)">🛟 fallback</span>`:a}
        ...
        <span class="right"><b class="ph-time">${t(n.ms)}</b>${n.tokens?e`<span class="ph-real" title="tokens reales de la fase">↓${c(n.tokens.in)} ↑${c(n.tokens.out)}</span>`:a}${this.estFor(n.phase)?e`<span class="ph-est" title="estimación preflight (sin gastar API)">est ↓${c(this.estFor(n.phase).estIn)} ↑${c(this.estFor(n.phase).estOut)}</span>`:a}</span>
      </div>
