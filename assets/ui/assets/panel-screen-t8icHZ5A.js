import{_ as e,a as t,d as n,f as r,g as i,h as a,i as o,l as s,m as c,n as l,p as u,r as d,s as f,t as p,v as m,y as h}from"./index-BlrRhlJz.js";import"./status-pill-Z7HHK8to.js";var g={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},_=e=>(...t)=>({_$litDirective$:e,values:t}),v=class{constructor(e){}get _$AU(){return this._$AM._$AU}_$AT(e,t,n){this._$Ct=e,this._$AM=t,this._$Ci=n}_$AS(e,t){return this.update(e,t)}update(e,t){return this.render(...t)}},{I:y}=h,b=e=>e.strings===void 0,x={},S=(e,t=x)=>e._$AH=t,C=_(class extends v{constructor(e){if(super(e),e.type!==g.PROPERTY&&e.type!==g.ATTRIBUTE&&e.type!==g.BOOLEAN_ATTRIBUTE)throw Error("The `live` directive is not allowed on child or event bindings");if(!b(e))throw Error("`live` bindings can only contain a single expression")}render(e){return e}update(t,[n]){if(n===e||n===i)return n;let r=t.element,a=t.name;if(t.type===g.PROPERTY){if(n===r[a])return e}else if(t.type===g.BOOLEAN_ATTRIBUTE){if(!!n===r.hasAttribute(a))return e}else if(t.type===g.ATTRIBUTE&&r.getAttribute(a)===n+``)return e;return S(t),n}}),w=class extends r{constructor(...e){super(...e),this.value=``,this.projId=``,this.placeholder=``,this.rows=3,this.open=!1,this.items=[],this.index=0,this.kind=`@`,this.query=``,this.tokStart=0,this.timer=0,this.seq=0,this.pendingCaret=null,this.skillsCache=null,this.skillsCacheProj=``}disconnectedCallback(){super.disconnectedCallback(),clearTimeout(this.timer)}emit(e){this.dispatchEvent(new CustomEvent(`cdr-input`,{detail:{value:e},bubbles:!0,composed:!0}))}close(){this.open=!1,this.items=[],clearTimeout(this.timer)}onInput(e){let t=e.target;this.emit(t.value);let n=t.value.slice(0,t.selectionStart).match(/(^|\s)(?:@([^\s@]*)|\/([^\s@/]*))$/);if(!n){this.close();return}let r=n[2]!==void 0;this.kind=r?`@`:`/`,this.query=r?n[2]:n[3],this.tokStart=t.selectionStart-this.query.length-1,this.open=!0,this.index=0,clearTimeout(this.timer),this.timer=window.setTimeout(()=>void this.fetchItems(),130)}async fetchItems(){let e=++this.seq,t=this.projId;try{let n=new URLSearchParams;if(t&&n.set(`project`,t),this.kind===`@`){n.set(`q`,this.query);let t=await(await fetch(`/api/files?`+n)).json();if(e!==this.seq)return;this.items=(t.files||[]).slice(0,30).map(e=>({v:e,label:e}))}else{if((!this.skillsCache||this.skillsCacheProj!==t)&&(this.skillsCache=(await(await fetch(`/api/skills?`+n)).json()).skills||[],this.skillsCacheProj=t),e!==this.seq)return;let r=this.query.toLowerCase();this.items=(this.skillsCache||[]).filter(e=>!r||e.name.toLowerCase().includes(r)).slice(0,20).map(e=>({v:e.name,label:e.name,sub:e.title}))}this.index=0}catch{this.items=[]}}onKey(e){this.open&&(e.key===`ArrowDown`?(e.preventDefault(),this.items.length&&(this.index=(this.index+1)%this.items.length)):e.key===`ArrowUp`?(e.preventDefault(),this.items.length&&(this.index=(this.index-1+this.items.length)%this.items.length)):(e.key===`Enter`||e.key===`Tab`)&&this.items.length?(e.preventDefault(),this.insert(this.items[this.index])):e.key===`Escape`&&(e.preventDefault(),this.close()))}insert(e){let t=this.querySelector(`textarea`);if(!t)return;let n=t.selectionStart,r=this.kind===`@`&&/\s/.test(e.v)?`"`+e.v+`"`:e.v,i=this.kind+r,a=t.value.slice(0,this.tokStart)+i+` `+t.value.slice(n),o=this.tokStart+i.length+1;this.value=a,this.pendingCaret=o,this.emit(a),this.close()}updated(){if(this.pendingCaret==null)return;let e=this.querySelector(`textarea`);e&&(e.focus(),e.setSelectionRange(this.pendingCaret,this.pendingCaret)),this.pendingCaret=null}render(){return m`
      <div class="mi-wrap">
        ...
        ${this.open?m`
          <ul id="mi-menu" class="mi-menu" role="listbox" aria-label=${this.kind===`@`?`ficheros`:`skills`}>
            ...
            ${this.items.length?this.items.map((e,t)=>m`
  ...
  "models": {
    ...
    "glm-v52": { "limit": { "context": 250000, "output": 16384 } }
      ...
      cifrarla: <code>"seal": true</code> o <code>conductor litellm login</code>. <strong>Nunca sale de tu máquina</strong>, no se registra ni se cachea.</p>`,s=(e?.byok??[]).length?m`<p class="inst-note">Leído de <code>~/.conductor/litellm.json</code> — tu catálogo declarado (${e.byok.length}):
      ...
      Comprueba QUÉ key hay dentro con <code>conductor litellm status</code> (huella, sin imprimirla).</p>`:i;return r?m`
      ...
      <details class="inst-panel inst-prompt" style="margin-top:.6rem">
        ...
        <div class="inst-body">
          ...
          ${o}
          ${t?m`<div class="inst-msg bad" role="status" aria-live="polite">${t}</div>`:i}
        </div>
      </details>`}planPanel(){let e=this.est?.checks??[],t=e.filter(e=>e.always),n=e.filter(e=>!e.always),r=new Set(this.phaseSel);return m`
      <div class="launch-plan" style="margin:.1rem 0 .2rem;padding:.7rem .9rem;border-left:3px solid var(--accent);background:var(--accentbg);border-radius:7px;color:var(--tx)">
        <div style="display:flex;align-items:baseline;justify-content:space-between;gap:.6rem">
          ...
          ${this.pipelineTouched?m`<button type="button" @click=${()=>this.resetPipeline()} style="background:none;border:none;padding:0;font-size:.74rem;color:var(--accent,#4f7cff);cursor:pointer;text-decoration:underline">restablecer plan propuesto</button>`:i}
        </div>
        ...
        <ul role="group" aria-label="Fases SDD del run" style="margin:.35rem 0 0;padding:0;list-style:none;font-size:.85rem;line-height:1.5">
          ${A.map(e=>{let t=k[e],n=j.includes(e),a=n||r.has(e);return m`<li style="padding:.12rem 0">
              <label style="display:flex;align-items:center;gap:.45rem;cursor:${n?`default`:`pointer`};${a?``:`opacity:.5`}">
                ...
                ${n?m`<span title="obligatoria — gobierno innegociable" aria-hidden="true">${l(`lock`)}</span>`:i}
                ${t?.artifact?m`<span class="muted" style="font-weight:400">→ ${t.artifact}</span>`:i}
                ${t?.gate?m`<span class="muted" style="font-weight:400">· ${t.gate}</span>`:i}
              </label>
            </li>`})}
        </ul>
        <div style="margin-top:.5rem;padding-top:.5rem;border-top:1px dashed var(--bd,#d8dee9)">
          <label style="display:flex;align-items:center;gap:.45rem;cursor:${this.est?.testCmd?`pointer`:`not-allowed`};${this.est?.testCmd?``:`opacity:.5`}" title=${this.est?.testCmd?`Ejecuta las pruebas REALES del proyecto ANTES de verify. Si fallan → ciclo fix → reintenta; si no pasan tras N intentos, el run queda BLOCKED.`:`No se detectó comando de pruebas en este proyecto`}>
            ...
            ${this.est?.testCmd?m`<span class="muted" style="font-weight:400">→ ejecutar pruebas del proyecto · <code style="font-size:.85em">${this.est.testCmd}</code></span>`:m`<span class="muted" style="font-weight:400">→ sin comando de pruebas detectado</span>`}
          </label>
        ...
        ${n.length?m`
          ...
          <ul style="margin:.2rem 0 0;padding-left:1.15rem;font-size:.82rem;line-height:1.5">
            ${n.map(e=>m`<li>${e.label}${e.why?m` <span class="muted">— ${e.why}</span>`:i}</li>`)}
          </ul>`:i}
      </div>`}render(){let e=this.scopeProjects(),t=this.metrics(e),n=this.modelOptions(),r=this.projects.flatMap(e=>(e.changes??[]).filter(e=>f(e.verdict)===`CURSO`).map(t=>({p:e,c:t}))),a=e.flatMap(e=>(e.changes??[]).filter(e=>f(e.verdict)!==`CURSO`).map(t=>({p:e,c:t}))),o=m`
      <form class="launch-form" @submit=${e=>void this.launch(e)}>
        ...
        ${this.atts.length?m`<div class="att-row">
          ${this.atts.map((e,t)=>m`<span class="att-chip"><img src=${e.data} alt="">${e.name}<button type="button" class="att-x" aria-label="Quitar ${e.name}" @click=${()=>{this.atts=this.atts.filter((e,n)=>n!==t)}}>×</button></span>`)}
        ...
        <div class="launch-meta">
          ...
          ${this.est?m`<details class="lm-estd"><summary class="lm-est">≈ ${d(this.est.total)} tokens · ${this.est.rows.length} fases <span class="muted">· preflight, sin API</span></summary>
            <table class="est-tab">${this.est.rows.map(e=>m`<tr><td>${e.phase}</td><td>↓ ${d(e.estIn)}</td><td>↑ ${d(e.estOut)}</td></tr>`)}</table>
            ${this.est.saved>0?m`<p class="est-saved">Ahorro estimado de ${d(this.est.saved)} tokens de entrada <span class="muted">(el planner no relee el repositorio)</span></p>`:i}
          </details>`:i}
        </div>
        ${n.length?m`
        <details class="inst-panel" style="margin-top:.6rem">
          ...
          <div class="inst-body">
            ...
            <details class="adv-phases">
              ...
              <div class="phase-grid" style="margin-top:.45rem">
                ${T.PHASES.map(e=>this.roleSelect(e,this.mPhases[e]??``,t=>{this.mPhases={...this.mPhases,[e]:t},this.preset=``}))}
              </div>
            ...
            <div class="frow" style="margin-top:.55rem;align-items:center">
              <button type="button" class="btn sm sec" ?disabled=${this.savingDefaults||!(this.mPlanner||this.mCoder||this.mReviewer||Object.values(this.mPhases).some(Boolean))} @click=${()=>void this.saveModelsDefault()} title="Escribe esta mezcla en openspec/conductor.json — será el default del EQUIPO para este proyecto (committeable)">${this.savingDefaults?`…`:m`${l(`save`)} Guardar como default del proyecto`}</button>
              ${this.saveDefaultsMsg?m`<span class="inst-msg ${this.saveDefaultsMsg.startsWith(`✓`)?`ok`:`bad`}" role="status" aria-live="polite" style="margin-top:0">${this.saveDefaultsMsg}</span>`:i}
            </div>
          </div>
        </details>`:i}
        ${this.byokForm()}
      </form>
      ${this.error?m`<p role="alert" style="color:var(--bad)">${this.error}</p>`:i}
    `,s=this.projects.find(e=>e.id===this.projId)??null,c=s&&s.openspec===!1?this.initPanel(s):o,u=this.attention();return m`
      ...
      <div class="apphdr"><h1>${s?s.name:`conductor`}</h1></div>
      ${u.length?m`<div class="attn" role="alert" aria-label="runs que esperan tu decisión">
        ${u.map(({p:e,c:t})=>m`<a class="attn-item" href="/run/${e.id}/${t.name}">${l(`pause`)} <b>${t.name}</b> espera tu decisión${this.projects.length>1?m` <span class="muted">· ${l(`folder`)} ${e.name}</span>`:i}<span class="attn-go">Abrir →</span></a>`)}
      </div>`:i}
      ...
      <section class="metrics-strip" aria-label="métricas del proyecto">
        <div class="cards">
          ...
          ${t.curso>0?m`<div class="card warn"><small>En curso</small><span>${t.curso}</span></div>`:i}
          <div class="card"><small>Tokens entrada</small><span>${t.measured?m`<i class="dir">↓</i>${d(t.tin)}`:`—`}</span></div>
          <div class="card"><small>Tokens salida</small><span>${t.measured?m`<i class="dir">↑</i>${d(t.tout)}`:`—`}</span></div>
          ${this.gh?m`<div class="card aic"><small>AI Credits</small><span>${this.gh.used}/${this.gh.entitlement}</span><div class="pbar ${this.gh.percentUsed>80?`warn`:``}"><i style="width:${Math.min(100,this.gh.percentUsed)}%"></i></div></div>`:i}
          ${this.usage?m`<div class="card"><small>Uso total LiteLLM</small><span>$${this.usage.spend.toFixed(2)}${this.usage.budget?m` <span class="muted" style="font-size:.8rem;font-weight:500">/ $${this.usage.budget.toFixed(0)}</span>`:i}</span>${this.usage.budget?m`<div class="pbar ${this.usage.spend/this.usage.budget>.8?`warn`:``}"><i style="width:${Math.min(100,this.usage.spend/this.usage.budget*100)}%"></i></div>`:i}</div>`:i}
        </div>
        ${this.version||!t.measured&&t.total?m`<p class="metrics-foot" title="versión del motor en uso">${this.version?m`motor v${this.version}`:i}${!t.measured&&t.total?m`${this.version?` · `:``}tokens sin medir en estos runs`:i}</p>`:i}
      </section>
      ...
      ${a.length>0||this.q.trim()?m`
        ...
        ${this.q.trim()?m`<p class="muted" style="font-size:.78rem;margin:.2rem 0 .6rem">${this.hits.length} resultado${this.hits.length===1?``:`s`} para "${this.q}"</p>${this.hits.map(e=>this.hitRow(e))}`:a.map(({p:e,c:t})=>this.runRow(e,t))}
      `:i}

      ${t.total===0?m`<p class="muted" style="margin-top:.5rem">Aún no hay runs. Lanza el primero arriba.</p>`:i}

      ${this.archived.length?m`
        <details class="arch">
          <summary>${l(`archive`)} Archivados <span class="muted">· ${this.archived.length}</span></summary>
          ${this.archived.map(e=>m`
            <div class="arch-row">
              <status-pill .verdict=${e.verdict}></status-pill>
              <span class="nm">${e.name}</span>
              ${e.project?m`<span class="proj">${l(`folder`)} ${e.project}</span>`:i}
              <span class="muted">${e.date??``} · ${e.phases} fases · ${e.request}</span>
            </div>`)}
        </details>`:i}
    `}hitRow(e){let t=m`<status-pill .verdict=${e.verdict}></status-pill><span class="nm">${e.name}</span>${e.archived?m`<span class="tag" title="archivado">${l(`archive`)}</span>`:i}${e.project?m`<span class="proj">${l(`folder`)} ${e.project}</span>`:i}<span class="muted snip">…${e.snippet}…</span>`;return m`<div class="hit-row">${e.archived?t:m`<a class="hit-main" href="/run/${e.projectId??this.defProjId}/${e.name}">${t}</a>`}</div>`}launchModelSummary(){let e=e=>e.replace(/^(byok|copilot):/,``),t=Object.entries(this.mPhases).filter(([,e])=>e),n=[this.mPlanner,this.mCoder,this.mReviewer,...t.map(([,e])=>e)].filter(Boolean);if(!n.length)return i;let r=t.map(([t,n])=>`${t} ${e(n)}`).join(` · `);return m`<span class="lm-model" title="modelo elegido por fase — cámbialo en «Modelo por fase»">Modelo: <b>${n.every(e=>e===n[0])&&!r?e(n[0]):[[this.mPlanner,this.mCoder,this.mReviewer].some(Boolean)?`planner ${e(this.mPlanner)||`—`} · coder ${e(this.mCoder)||`—`} · reviewer ${e(this.mReviewer)||`—`}`:``,r].filter(Boolean).join(` · `)}</b></span>`}mixNote(){let e=e=>e?e.startsWith(`byok:`)?`LiteLLM`:e.startsWith(`copilot:`)?`Copilot`:`sesión`:``,t=[...new Set([this.mPlanner,this.mCoder,this.mReviewer,...Object.values(this.mPhases)].map(e).filter(Boolean))];return t.length<2?i:m`<div class="muted" style="font-size:.72rem;margin-top:.5rem">proveedores: ${t.join(` + `)}</div>`}roleSelect(e,t,n){let r=this.models,a=r?.copilot??[],o=r?.byok??[],s=!!r?.byokCreds,c=e=>e.startsWith(`claude`)?`Claude`:e.startsWith(`gpt`)?`GPT`:e.startsWith(`gemini`)?`Gemini`:`Otros`,l=e=>r?.vendors?.[e]??c(e),u=r?.tiers??{},d=[`Anthropic`,`Claude`,`OpenAI`,`GPT`,`Google`,`Gemini`],f=[...new Set(a.map(l))].sort((e,t)=>{let n=d.indexOf(e),r=d.indexOf(t);return(n<0?99:n)-(r<0?99:r)||e.localeCompare(t)}).map(e=>[e,a.filter(t=>l(t)===e).sort((e,t)=>this.tierRank(u[t])-this.tierRank(u[e])||t.localeCompare(e))]).filter(([,e])=>e.length),p=r?.copilotPending;return m`<label class="fl" style="flex:1">${e}<select .value=${t} title=${r?`Copilot: ${r.copilotSource} · LiteLLM: ${r.byokSource}`:``} @change=${e=>n(e.target.value)}>
      <option value="">Recomendado (conductor elige)</option>
      ${f.length?f.map(([e,t])=>m`<optgroup label="Copilot · ${e}${p?` · vistos en tus runs`:``}">${t.map(e=>m`<option value="copilot:${e}">${this.modelLabel(e,`copilot`)}</option>`)}</optgroup>`):m`<option value="" disabled>catálogo Copilot aún no disponible</option>`}
      ${s&&o.length?m`<optgroup label="LiteLLM · 0 créditos">${o.map(e=>m`<option value="byok:${e}">${this.modelLabel(e,`byok`)}</option>`)}</optgroup>`:i}
    </select></label>`}runRow(e,t){return m`
      <div class="run-row ${f(t.verdict)===`CURSO`?`run-active`:``}">
        <div class="run-l">
          <a class="main" href="/run/${e.id}/${t.name}">
            <span class="nm">${t.name} <status-pill .verdict=${t.pending?`EN PAUSA`:t.verdict}></status-pill>${t.pending?m`<span class="pill CURSO" title="el run espera tu revisión">${l(`pause`)} tu decisión</span>`:i}</span>
            <span class="rq">${t.request}</span>
            <span class="proj">${l(`folder`)} ${e.name}</span>
          </a>
          <div class="run-foot">
            <span class="meta">${t.phases} fases${(t.tokens?.in??0)+(t.tokens?.out??0)>0?m` · ↓ ${d(t.tokens?.in)} entrada · ↑ ${d(t.tokens?.out)} salida`:i}</span>
            ...
            ${t.resumable?m`<button class="btn sm sec" @click=${()=>void this.resume(e,t)} aria-label="reanudar ${t.name}">${l(`play`)} Reanudar</button>`:i}
            ${!t.resumable&&f(t.verdict)!==`CURSO`&&t.request?m`<button class="btn sm sec" @click=${()=>this.reuse(t)} title="rellena el formulario con esta petición para lanzar una variante">${l(`redo`)} Reutilizar</button>`:i}
            ${t.hasDashboard?m`<a class="btn sm sec" href="/artifact/${e.id}/${t.name}/dashboard.html" target="_blank" aria-label="informe de ${t.name}" title="Informe del run: fases con modelo y tokens reales vs estimados, gate y linaje requisito→código→test.">${l(`report`)} Informe</a>`:i}
            ${t.phases>0?m`<a class="btn sm sec" href="/api/run/${e.id}/${t.name}/aiact" target="_blank" aria-label="AI Act de ${t.name}" title="El acta de «quién hizo qué» que pide el reglamento europeo de IA: modelos y papel por fase, aprobaciones humanas y sello.">${l(`shield`)} AI Act</a>`:i}
          </div>
        </div>
        <a class="run-open" href="/run/${e.id}/${t.name}" tabindex="-1" aria-hidden="true"><span class="chev" aria-hidden="true"></span></a>
      </div>`}};p([u()],M.prototype,`projects`,void 0),p([u()],M.prototype,`gh`,void 0),p([u()],M.prototype,`usage`,void 0),p([u()],M.prototype,`version`,void 0),p([u()],M.prototype,`models`,void 0),p([u()],M.prototype,`req`,void 0),p([u()],M.prototype,`name`,void 0),p([u()],M.prototype,`complexity`,void 0),p([u()],M.prototype,`auto`,void 0),p([u()],M.prototype,`projId`,void 0),p([u()],M.prototype,`mPlanner`,void 0),p([u()],M.prototype,`mCoder`,void 0),p([u()],M.prototype,`mReviewer`,void 0),p([u()],M.prototype,`mPhases`,void 0),p([u()],M.prototype,`preset`,void 0),p([u()],M.prototype,`busy`,void 0),p([u()],M.prototype,`error`,void 0),p([u()],M.prototype,`atts`,void 0),p([u()],M.prototype,`savingDefaults`,void 0),p([u()],M.prototype,`saveDefaultsMsg`,void 0),p([u()],M.prototype,`est`,void 0),p([u()],M.prototype,`phaseSel`,void 0),p([u()],M.prototype,`runTests`,void 0),p([u()],M.prototype,`initBusy`,void 0),p([u()],M.prototype,`q`,void 0),p([u()],M.prototype,`hits`,void 0),p([u()],M.prototype,`archived`,void 0),M=T=p([a(`panel-screen`)],M);export{M as PanelScreen,O as smartName};