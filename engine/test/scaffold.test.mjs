...
await test('scaffold: crea conductor.json + project.md + .copilotignore + .gitignore; NUNCA pisa lo del usuario', () => {
  ...
  // SIN config.yaml (decisión tras análisis): el marcador solo servía para que el CLI oficial de
  // OpenSpec reconociera el repo — y su `archive` propio archivaría SIN mover evidencia ni ledger
  // (dos archivadores = incoherencia). Quien lo quiera lo crea a mano; isSdd() lo sigue reconociendo.
  assert(!existsSync(join(OS, 'config.yaml')), 'SIN config.yaml — ni espejo ni marcador');
