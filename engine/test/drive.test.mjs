...
await test('drive(vía honesta): una decisión con source:"chat" queda como human-chat — jamás se disfraza de clic humano', async () => {
  fresh();
  const changeDir = join(TMP, 'openspec', 'changes', 'via');
  const r = await drive({
