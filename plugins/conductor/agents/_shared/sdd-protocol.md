...
> **OpenSpec vs Conductor**: `schema`, `context`, `rules` are OpenSpec standard fields. Everything under `x-conductor` is a Conductor extension. Project context (architecture, directories, conventions) lives in platform instruction files (`.github/instructions/`, `.claude/rules/`) generated by `/instructions` — NOT in openspec/. config.yaml holds only executable pipeline config (hooks, testing commands, TDD mode). No duplication between instruction files and config.yaml.
