...
| `/sdd-impex` | (SAP Commerce) Generar ImpEx data con validaciones |
...
| `config.yaml` | `strict_tdd: true` (Salesforce exige 95%+ de cobertura) |
...
| Fase | Detalles |
|------|----------|
| `/sdd-init` detecta | Java (`extensioninfo.xml` identifica Hybris), Maven/Ant |
| `config.yaml` | `strict_tdd: false` (muchos proyectos Hybris carecen de unit tests) |
| `/sdd-instructions` genera | `hybris.instructions.md` (applyTo: `**/*.java,**/*.xml,**/*.impex`) |
| Ediciones manuales **críticas** | "Capas: Controller/Facade a Service a DAO". "Sufijos obligatorios: Service, Facade, Controller, Dao, Converter, Populator". "Prefijo de proyecto en clases custom". "Nunca modificar clases core de SAP — extender + override en spring.xml". "ImpEx: respetar formato y tipos estrictamente". "Cronjobs en vez de Business Process para tareas asíncronas". Usar modo interactivo (sin `--auto`) — Hybris es frágil, revisar antes de aplicar |
